...
def write_epub(src_dir, out_path):
    ...
    with ZipFile(out_path, 'w', compression=ZIP_DEFLATED, compresslevel=9) as z:
        if os.path.isfile(mimetype_path):
            z.writestr('mimetype', open(mimetype_path, 'rb').read(), compress_type=ZIP_STORED)
...
def rasterize_svg_string(svg_text, width, height, font_spec=None, font_is_url=False):
    ...
    s = svg_text if isinstance(svg_text, str) else svg_text.decode('utf-8', errors='ignore')
    ...
    if m:
        ...
        has_height = bool(re.search(r'\bheight\s*=\s*"?\w+', attrs, flags=re.IGNORECASE))
        has_viewbox = bool(re.search(r'\bviewBox\s*=|\bviewbox\s*=', attrs, flags=re.IGNORECASE))
        if not (has_width and has_height) and not has_viewbox:
            s = re.sub(r'(<svg\b)([^>]*)>', r"\1\2 width=\"%d\" height=\"%d\" viewBox=\"0 0 %d %d\">" % (width, height, width, height), s, count=1, flags=re.IGNORECASE)

    # inject style block (font embedding or family) immediately after opening <svg>
    if style_block:
        s = re.sub(r'(<svg\b[^>]*>)', r"\1" + style_block, s, count=1, flags=re.IGNORECASE)
    ...
    try:
        ...
        try:
            s2 = re.sub(r'(<svg\b)([^>]*)>', r"\1\2 width=\"%d\" height=\"%d\" viewBox=\"0 0 %d %d\">" % (width, height, width, height), s, count=1, flags=re.IGNORECASE)
...
def process_single_epub(epub_path, out_path, timeout, font_spec=None):
    ...
    try:
        ...
        if os.path.isfile(cover_path):
            ...
            m = re.search(r'<svg\b.*?</svg>', cover_text, flags=re.DOTALL | re.IGNORECASE)
            if m:
                svg_string = m.group(0)
                img = rasterize_svg_string(svg_string, width=1600, height=2560, font_spec=font_for_svg, font_is_url=font_is_url)
                out_img_path = os.path.join(images_dir, 'cover.jpeg')
                img.save(out_img_path, format='JPEG', quality=100, subsampling=0, optimize=True, progressive=False)
...
def main():
    ...
    p.add_argument('-f', '--font', default='/usr/share/fonts/opentype/noto/NotoSansCJK-Bold', help='Path to TTF file or font family name (default: %(default)s)')
    ...
    with ThreadPoolExecutor(max_workers=args.jobs) as ex:
        ...
        for epub in epubs:
            ...
            futures.append(ex.submit(process_single_epub, epub, out_path, args.timeout, args.font))
